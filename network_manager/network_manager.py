...
class NetworkManager:
    def __init__(self):
        self.connections = {}   # (container_a, container_b)
        self.servers = {}       # container_id -> thread
    ...
    # ----------------------------
    def start_container_network(self, container_id, port):
        ...
        def server():
            ...
            # FIX: reuse address (critical for Termux)
    ...
    def _handle_client(self, container_id, client_socket):
        try:
    ...
    # Connect Containers
    # ----------------------------
    def connect_containers(self, container_a, container_b):
        self.connections[(container_a, container_b)] = True
        self.connections[(container_b, container_a)] = True

    def disconnect_containers(self, container_a, container_b):
    ...
    # Send Message
    # ----------------------------
    def send_message(self, from_container, to_container, message):
        ...
        if to_container not in self.ports:
            raise RuntimeError(f"{to_container} has no network")
    ...
    # ----------------------------
    def map_port(self, container_id, host_port, container_port):
        def forwarder():
            ...
            while self.running:
                try:
                    ...
                    threading.Thread(
                        target=self._pipe, args=(client, forward), daemon=True
                    ).start()

                    threading.Thread(
                        target=self._pipe, args=(forward, client), daemon=True
                    ).start()
    ...
    # ----------------------------
    # Container Web Server (NEW)
    # ----------------------------
    def serve_container_web(self, container_id, fs_manager, port=8080):
    ...
    def list_networks(self):
        return {
            cid: {
                "port": self.ports.get(cid),
                "active": cid in self.servers
            }
